...
int main (int argc,char *argv [])
		...
		switch (type)
			...
			case DBTableFieldInt:		
				if ((length != 1) && (length != 2) && (length != 4))
					{ CMmsgPrint (CMmsgUsrError,"Invalid field length"); delete data; return (CMfailed); }
				break;
	...
	if (dDecimals != DBFault)
		...
		if (type != DBTableFieldFloat)
			{ CMmsgPrint (CMmsgUsrError,"Invalid field type for decimals!"); delete data; return (CMfailed); }
		dDecimals = 2;
		}
