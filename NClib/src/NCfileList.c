...
static int _NCfileFilter(const struct dirent *dirent) {
    regmatch_t pmatch;
    return (regexec(&_NCfileListRegex, dirent->d_name, 1, &pmatch, REG_NOTBOL | REG_NOTEOL) == REG_NOMATCH ? false
                                                                                                           : true);
}
...
char **NCfileList(const char *template, size_t *n) {
    ...
    if (regcomp(&_NCfileListRegex, pattern, REG_EXTENDED | REG_NOSUB) != 0) {
        CMmsgPrint(CMmsgAppError, "Regular expression error in: %s %d", __FILE__, __LINE__);
        goto ABORT;
    }
    regexIsSet = true;

    if ((ndirents = scandir(path, &dirents, _NCfileFilter, alphasort)) < 0) {
        CMmsgPrint(CMmsgSysError, "Directory scanning error in: %s %d", __FILE__, __LINE__);
        goto ABORT;
    }
    if (ndirents < 1) {
        CMmsgPrint(CMmsgUsrError, "No file is matching pattern [%s]", template);
        goto ABORT;
    }
    ...
    for (i = 0; i < ndirents; ++i) {
        if ((fileList[i] = (char *) malloc(strLength + strlen(dirents[i]->d_name) + 1)) == (char *) NULL) {
            CMmsgPrint(CMmsgSysError, "Memory allocation error in: NCfileList ()");
            goto ABORT;
        }
    ...
    *n = ndirents;
    ...
    if (dirents != (struct dirent **) NULL) {
        for (i = 0; i < ndirents; ++i) free(fileList[i]);
        free(fileList);
    }
    if (regexIsSet) regfree(&_NCfileListRegex);
    *n = 0;
