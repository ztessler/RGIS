...
int main (int argc,char *argv [])
	...
	if (yearFieldName != (char *) NULL)
		...
		if ((yearField = table->Field (yearFieldName)) == (DBObjTableField *) NULL)
			{ CMmsgPrint (CMmsgUsrError,"Invalid year field [%s]!",yearFieldName); delete data; return (CMfailed); }
		}

	if (monthFieldName != (char *) NULL)
		{
		if ((monthField = table->Field (monthFieldName)) == (DBObjTableField *) NULL)
			{ CMmsgPrint (CMmsgUsrError,"Invalid month field [%s]!",monthFieldName); delete data; return (CMfailed); }
		}
