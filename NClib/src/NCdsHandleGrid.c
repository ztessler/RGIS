...
NCstate NCdsHandleGridDefine(NCdsHandleGrid_t *grid, int *ncids, size_t n) {
    ...
    if ((grid->DataType != NCtypeGCont) && (grid->DataType != NCtypeGDisc)) {
        CMmsgPrint(CMmsgAppError, "Invalid data in: %s %d", __FILE__, __LINE__);
        goto ABORT;
    }
    ...
    for (i = 0; i < n; ++i) {
        if ((status = nc_inq_dimid(ncids[i], NCnameDNTime, &dimid)) != NC_NOERR) {
            NCprintNCError (status, "NCdsHandleGridDefine");
            goto ABORT;
        }
        ...
        if (grid->DoTUnit && (status != NC_NOERR)) grid->DoTUnit = false;
            ...
            CMmsgPrint(CMmsgAppError, "Invalid time unit string [%s] in: %s %d", __FILE__, __LINE__, unitstr);
            grid->DoTUnit = false;
        }
        else {
        ...
        if (tnum > 0) {
            if (((grid->Times = (double *) realloc(grid->Times, (grid->TNum + tnum) * sizeof(double))) ==
                 (double *) NULL) ||
                ((grid->NCindex = (size_t *) realloc(grid->NCindex, (grid->TNum + tnum) * sizeof(size_t))) ==
                 (size_t *) NULL)) {
                CMmsgPrint(CMmsgSysError, "Memory allocation error in: %s %d", __FILE__, __LINE__);
                goto ABORT;
            }
        ...
        if ((status = nc_inq_dimid(ncids[i], NCnameDNLevel, &dimid)) == NC_NOERR) {
            ...
            if (i == 0) {
                ...
                CMmsgPrint(CMmsgAppError, "Inconsistent NetCDF bundle in: %s %d", __FILE__, __LINE__);
                goto ABORT;
            }

            if (grid->DoLUnit && (status != NC_NOERR)) grid->DoLUnit = false;
            else if (utScan(unitstr, grid->LUnits + i) != 0) {
                CMmsgPrint(CMmsgAppError, "Invalid level unit string [%s] in: %s %d", __FILE__, __LINE__, unitstr);
                grid->DoLUnit = false;
            }
        }
        else grid->LNum = 1;
        ...
        for (i = 1; i < grid->TNum; ++i) {
            if (grid->DoTUnit && ((j = grid->NCindex[i]) != 0)) {
                if (utConvert(grid->TUnits + j, grid->TUnits, &scale, &offset) != 0) {
                    CMmsgPrint(CMmsgAppError, "Invalid time conversion in: %s %d", __FILE__, __LINE__);
                    break;
                }
            }
            ...
            if (i == 1) delta = x1 - x0;
                ...
                CMmsgPrint(CMmsgAppError, "Irregular time step in: %s %d", __FILE__, __LINE__);
                break;
            }
            x0 = x1;
        }
        ...
        for (i = 1; i < grid->LNum; ++i) {
            if (grid->DoLUnit && ((j = grid->NCindex[i]) != 0)) {
                if (utConvert(grid->LUnits + j, grid->LUnits, &scale, &offset) != 0) {
                    CMmsgPrint(CMmsgAppError, "Invalid level conversion in: %s %d", __FILE__, __LINE__);
                    break;
                }
            }
...
void NCdsHandleGridClear(NCdsHandleGrid_t *grid) {
    ...
    grid->LNum = grid->TNum = (size_t) 0;
    if (grid->LVarIds != (int *) NULL) {
        free(grid->LVarIds);
        grid->LVarIds = (int *) NULL;
    }
    if (grid->TVarIds != (int *) NULL) {
        free(grid->TVarIds);
        grid->TVarIds = (int *) NULL;
    }
    if (grid->Times != (double *) NULL) {
        free(grid->Times);
        grid->Times = (double *) NULL;
    }
    if (grid->Levels != (double *) NULL) {
        free(grid->Levels);
        grid->Levels = (double *) NULL;
    }
    if (grid->LUnits != (utUnit *) NULL) {
        free(grid->LUnits);
        grid->LUnits = (utUnit *) NULL;
    }
    if (grid->TUnits != (utUnit *) NULL) {
        free(grid->TUnits);
        grid->TUnits = (utUnit *) NULL;
    }
}
