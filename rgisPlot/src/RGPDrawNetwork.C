...
DBInt RGPDrawNetwork(DBInt mode, DBInt *entryNum, DBObjData *netData) {
    ...
    do {
        ...
        if (i < (int) strlen(charBuffer)) {
            ...
            if ((scaleMode = CMoptLookup(scaleModes, charBuffer, true)) != DBFault) {
                if ((sscanf(charBuffer + i + 1, "%f,%f", &minValue, &maxValue) == 2) &&
                    (fabs(maxValue - minValue) > 0.0))
                    break;
                if (RGPPrintError(mode, *entryNum, "Width range input error")) {
                    ret = DBFault;
                    goto Stop;
                }
            }
    ...
    if (legendMode == 0)
        do {
            RGPPrintMessage(mode, entryNum,
                            "Legend position,title, and values [ll|lr|ul|ur,title,value1,value2,...,valueN]:");
            ...
            if (i < buffLen) {
                ...
                if ((legendPos = CMoptLookup(legendPositions, charBuffer, true)) != DBFault) {
                    ...
                    if (i < buffLen) {
                        ...
                        if (RGPPrintError(mode, *entryNum, "Legend values input error")) {
                            ret = DBFault;
                            goto Stop;
                        }
                    }
    ...
    for (recID = 0; recID < netIF->CellNum(); ++recID) {
        ...
        switch (scaleMode) {
            default:
            case 0:
                wValue = (wValue - minValue) / (maxValue - minValue);
                break;
            case 1:
                wValue = log10(wValue - minValue) / log10(maxValue - minValue);
                break;
        }
        ...
        if (basinID != netIF->CellBasinID(cellRec)) {
            ...
            switch (symbolMode) {
                default:
                case 1:
                    foreground = netIF->Color(basinRec);
                    break;
                case 2:
                    foreground = netIF->ItemForeground(basinRec);
                    break;
                case 3:
                    foreground = 1;
                    break;
    ...
    if (legendMode == 0) {
        ...
        for (i = 0; i < legendNum; ++i) {
            switch (scaleMode) {
                default:
                case 0:
                    wValue = (lValues[i] - minValue) / (maxValue - minValue);
                    break;
                case 1:
                    wValue = log10(lValues[i] - minValue) / log10(maxValue - minValue);
                    break;
            }
            wValue = wValue * RGPWidthConstant;
            width = (int) (ceil((double) wValue));
