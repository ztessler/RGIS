	...
	for (argPos = 1;argPos < argNum; )
		...
		if (CMargTest (argv [argPos],"-S","--sample"))
			...
			if ((argNum = CMargShiftLeft(argPos,argv,argc)) <= argPos)
				{ CMmsgPrint (CMmsgUsrError, "%s: Missing sampling coverage!",CMfileName (argv [0])); return (CMfailed); }
			sample = argv [argPos];
		...
		if (CMargTest (argv [argPos],"-L","--layout"))
			...
			if ((tblLayout = CMoptLookup (layoutTexts, argv [argPos],true)) == CMfailed)
				{ CMmsgPrint (CMmsgUsrError, "%s: Invalid table layout [%s]!",CMfileName (argv [0]), argv [argPos]); return (CMfailed); }
