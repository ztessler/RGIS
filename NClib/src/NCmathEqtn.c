...
static char *funcNames[] = {"abs", "sin", "asin", "cos", "acos", "tan", "atan", "ln", "log", "floor", "ceil",
                            (char *) NULL};
...
static Oper getOper(char expr[], int *i) {
    ...
    while (c >= s) {
        if ((expr[c - 1] == ' ') && (expr[c + 1] == ' ')) { // Must check for Add/Sub first
            if (expr[c] == '+') {
                *i = c;
                return ADD;
            }
    ...
    if ((expr[c] == ')') || (expr[c] == ']') || (expr[c] == '}')) c = NCstringEndPar(expr, c) - 2;
    while (c >= s) {
        if ((expr[c - 1] == ' ') && (expr[c + 1] == ' ')) {// then check for Mul/Div
            if (expr[c] == '*') {
                *i = c;
                return MUL;
            }
    ...
    if ((expr[c] == ')') || (expr[c] == ']') || (expr[c] == '}')) c = NCstringEndPar(expr, c) - 2;
    while (c >= s) {
        if ((expr[c - 1] == ' ') && (expr[c + 1] == ' ')) {// finally do Ecp
            if (expr[c] == '^') {
                *i = c;
                return EXP;
            }
...
static double (*getFunc(char expr[], int *i))(double) { // returns a pointer to a double function(double)
    int strLen = strlen(expr), j;
    if ((expr[*i] == '(') || (expr[*i] == '[') || (expr[*i] == '{')) *i = NCstringEndPar(expr, *i) + 2;
    while (*i < strLen) {
        for (j = 0; j < numFunc; j++)
            if (NCstringMatch(expr, *i, funcNames[j])) {
                *i += strlen(funcNames[j]) + 1;
                return functions[j];
            }
        (*i)++;
        if ((expr[*i] == '(') || (expr[*i] == '[') || (expr[*i] == '{')) *i = NCstringEndPar(expr, *i) + 2;
...
int NCmathAddVar(int colnum, char *name, bool vary) {
    ...
    if ((_Vars[_VarNum].name = (char *) malloc(sizeof(char) * (strlen(name) + 1))) == (char *) NULL) {
        ...
        strcpy(_Vars[_VarNum].name, name);
...
void NCmathFreeVars() {
    int i;
    for (i = 0; i < _VarNum; i++) free(_Vars[i].name);
    free(_Vars);
}
...
TreeNode_t *mkTree(char expr[]) {
    ...
    TreeNode_t *head;
    if (expr == (char *) NULL) {
        if (GetDebug()) CMmsgPrint(CMmsgUsrError, "mkTree(): Null string passed!");
    ...
    if (expr[0] == '(') {
        ...
        if ((head->oper = getOper(expr, &i)) != NOOP) {
            if (GetDebug())
                CMmsgPrint(CMmsgUsrError, "mkTree(): PAR:i='%d' i-1='%c' i='%c' i+1='%c'", i, expr[i - 1], expr[i],
                           expr[i + 1]);
            ...
            if ((head->func = getFunc(expr, &i)) != functions[numFunc]) {
                ...
                if (_Vars[i].vary) {
                    head->type = VAR;
                    head->var = &(_Vars[i].val);
                }
                else {
                    head->type = CONST;
                    head->cons = _Vars[i].val;
                }
                ...
                CMmsgPrint(CMmsgUsrError, "mkTree(): PAR:Unrecognized operator: strLen:%d i:%d '%s'", strLen, i, expr);
                abort();
            }
        }
    } else {
        ...
        if ((head->oper = getOper(expr, &i)) != NOOP) {
            if (GetDebug())
                CMmsgPrint(CMmsgUsrError, "mkTree(): NOPAR:i='%d' i-1='%c' i='%c' i+1='%c'", i, expr[i - 1], expr[i],
                           expr[i + 1]);
            head->left = mkTree(NCstringSubstr(expr, 0, i - 2));
            head->right = mkTree(NCstringSubstr(expr, i + 2, strLen - 1));
            head->type = OPER;
            ...
            if ((head->func = getFunc(expr, &i)) != functions[numFunc]) {
                if (GetDebug())
                    CMmsgPrint(CMmsgUsrError, "mkTree(): NOPAR:i='%d' i-1='%c' i='%c' i+1='%c'", i, expr[i - 1],
                               expr[i], expr[i + 1]);
                head->left = NULL;
                head->right = mkTree(NCstringSubstr(expr, i, NCstringEndPar(expr, i)));
                head->type = FUNC;
                ...
                CMmsgPrint(CMmsgUsrError, "mkTree(): NOPAR: Unrecognized operator: strLen:%d i:%d '%s'", strLen, i,
                           expr);
                abort();
            }
        }
...
IneqNode_t *mkTreeI(char expr[]) {
    ...
    char *temp;
    if (expr == (char *) NULL) {
        if (GetDebug()) CMmsgPrint(CMmsgUsrError, "mkTreeI(): Null string passed!");
        return NULL;
    }
    ...
    if ((head->equal = getIneq(expr, &i)) != NOEQ) {
        if (GetDebug())
            CMmsgPrint(CMmsgUsrError, "mkTreeI(): PAR:i='%d' i-1='%c' i='%c' i+1='%c'", i, expr[i - 1], expr[i],
                       expr[i + 1]);
        if ((temp = NCstringSubstr(expr, 0, i - 1)) == (char *) NULL) {
            CMmsgPrint(CMmsgUsrError, "Problem encountered with NCstringSubstr(%s, %d, %d)!", expr, 0, i - 1);
            return ((IneqNode_t *) NULL);
        }
        s = (expr[i + 1] == ' ') ? i + 1 : i + 2;
        while (NCstringStripch(&temp, ' ') || NCstringStripbr(&temp));
        i = 0;
        if (getIneq(temp, &i) == NOEQ) {
            head->lTree = true;
            head->lhead = mkTree(temp);
        } else {
            head->lTree = false;
            head->left = mkTreeI(temp);
        }
        if ((temp = NCstringSubstr(expr, s, strLen)) == (char *) NULL) {
            CMmsgPrint(CMmsgUsrError, "Problem encountered with NCstringSubstr(%s, %d, %d)!", expr, s, strLen);
            return ((IneqNode_t *) NULL);
        }
        while (NCstringStripch(&temp, ' ') || NCstringStripbr(&temp));
        i = 0;
        if (getIneq(temp, &i) == NOEQ) {
            head->rTree = true;
            head->rhead = mkTree(temp);
        } else {
            head->rTree = false;
            head->right = mkTreeI(temp);
        }
    }
    ...
    if (s != NULL) {
        if (s->lTree) {
            ...
            if (s->rTree) {
                ...
                if ((Calculate(s->lhead) != FLOAT_NOVALUE) &&
                    ((s->right->equal == TRUE) || (s->right->equal == FALSE))) {
                    s->equal = CalculateI(s);
                    delTree(s->lhead);
                    s->lhead = (TreeNode_t *) NULL;
                    delTreeI(s->right);
                    s->right = (IneqNode_t *) NULL;
                }
            }
        } else {
            if (s->rTree) {
                ...
                if ((Calculate(s->rhead) != FLOAT_NOVALUE) && ((s->left->equal == TRUE) || (s->left->equal == FALSE))) {
                    s->equal = CalculateI(s);
                    delTreeI(s->left);
                    s->left = (IneqNode_t *) NULL;
                    delTree(s->rhead);
                    s->rhead = (TreeNode_t *) NULL;
                }
            } else {
...
double Calculate(TreeNode_t *s) {
    ...
    if (s != NULL) {
        switch (s->type) {
            case CONST:
                if (GetDebug()) {
                    fprintf(stderr, "%f [const] = ", s->cons);
                    printInorder(s, stderr);
                }
                return s->cons;
            case OPER:
                ...
                if (NCmathEqualValues(right, FLOAT_NOVALUE) ||
                    NCmathEqualValues(left, FLOAT_NOVALUE))
                    return FLOAT_NOVALUE;
                switch (s->oper) {
                    case ADD:
                        if (GetDebug()) {
                            fprintf(stderr, "%f [oper] = ", left + right);
                            printInorder(s, stderr);
                        }
                        return left + right;
                    case SUB:
                    ...
                    case MOD:
                        if (NCmathEqualValues(right, (double) ((int) right)) &&
                            ...
                            if (GetDebug()) {
                                fprintf(stderr, "%i [oper] = ", (int) left % (int) right);
                                printInorder(s, stderr);
                            }
                    ...
                    case EXP:
                        if (GetDebug()) {
                            fprintf(stderr, "%f [oper] = ", pow(Calculate(s->left), Calculate(s->right)));
                            printInorder(s, stderr);
                        }
                        return pow(Calculate(s->left), Calculate(s->right));
                    default:
                        if (GetDebug()) fprintf(stderr, "BADOPER");
                        return FLOAT_NOVALUE;
                }
                break;
            case VAR:
                if (GetDebug()) {
                    CMmsgPrint(CMmsgUsrError, "%f [var] = ", *(s->var));
                    printInorder(s, stderr);
                }
            ...
            case FUNC:
                ...
                if ((GetDebug()) && (s->func == functions[numFunc]))
                    CMmsgPrint(CMmsgUsrError, "WARN: Bad function used!");
                if (GetDebug()) {
                    CMmsgPrint(CMmsgUsrError, "%f [func] = ", s->func(right));
                    printInorder(s, stderr);
                }
                return s->func(right);
                break;
