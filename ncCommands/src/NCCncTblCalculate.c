...
static void doHelp(const char *progName) {
    ...
    CMmsgPrint(CMmsgUsrError,
               "  -f,--file <FILENAME>              => specify a tab-delimited or NetCDF file to read data from");
    CMmsgPrint(CMmsgUsrError,
               "  -o,--output <FILENAME>            => specify the output file, which will always be ASCII tab-delimited!");
    ...
    CMmsgPrint(CMmsgUsrError,
               "  -v,--variable <VARNAME>=<double>  => define a static variable to be used in the expression");
    CMmsgPrint(CMmsgUsrError,
               "  -r,--rename <COLNAME>             => give a name for results column will be put in (default: \"RESULT\")");
...
void NCcloseFile(Files_t *file) {
    if (file->ncTYPE) {
        if (nc_close(file->ncid) != NC_NOERR) {
            CMmsgPrint(CMmsgUsrError, "Error closing file: %s!", file->fname);
            abort();
        }
...
int main(int argc, char *argv[]) {
    ...
    for (argPos = 1; argPos < argNum;) {
        if (CMargTest(argv[argPos], "-d", "--debug")) {
            SetDebug();
            if ((argNum = CMargShiftLeft(argPos, argv, argc)) <= argPos) break; else continue;
        }
        if (CMargTest(argv[argPos], "-l", "--lisp")) {
            setLisp();
            if ((argNum = CMargShiftLeft(argPos, argv, argc)) <= argPos) break; else continue;
        }
        ...
        if (CMargTest(argv[argPos], "-t", "--table")) {
            ...
            if ((inFile.ncTYPE != false) && (inFile.file != (FILE *) NULL)) {
                CMmsgPrint(CMmsgUsrError, "-t flag must precede -f flag!");
                cleanup(CMfailed);
            }
        ...
        if (CMargTest(argv[argPos], "-f", "--file")) {
            if ((argNum = CMargShiftLeft(argPos, argv, argc)) <= argPos) {
                CMmsgPrint(CMmsgUsrError, "Missing file!");
                cleanup(CMfailed);
            }
            if ((inFile.ncTYPE != false) && (inFile.file != (FILE *) NULL)) {
                CMmsgPrint(CMmsgUsrError, "multiple -f flags!");
                cleanup(CMfailed);
            }
            ...
            if (nc_open(inFile.fname, NC_WRITE, &i) == NC_NOERR) {
                ...
                if ((inFile.tbl = NCtableOpen(i, tablename)) == (NCtable_t *) NULL) {
                    CMmsgPrint(CMmsgUsrError, "Error encountered!");
                    cleanup(CMfailed);
                }
            }
                ...
                if (inFile.curRow == (char *) NULL) {
                    CMmsgPrint(CMmsgUsrError, "Empty File '%s'", inFile.fname);
                    cleanup(CMfailed);
                }
                else {
                ...
                CMmsgPrint(CMmsgUsrError, "Cannot open file '%s'!", inFile.fname);
                cleanup(CMfailed);
            }
            if ((argNum = CMargShiftLeft(argPos, argv, argc)) <= argPos) break;
            continue;
        ...
        if (CMargTest(argv[argPos], "-o", "--output")) {
            ...
            if (strcmp(argv[argPos], "-") == 0) outFile = stdout;
                ...
                CMmsgPrint(CMmsgUsrError, "Error opening file '%s' for writing!", argv[argPos]);
                cleanup(CMfailed);
            }
            if ((argNum = CMargShiftLeft(argPos, argv, argc)) <= argPos) break;
            continue;
    ...
    for (argPos = 1; argPos < argNum;) {
        if (CMargTest(argv[argPos], "-v", "--variable")) { // constant variable
            if ((argNum = CMargShiftLeft(argPos, argv, argc)) <= argPos) {
                CMmsgPrint(CMmsgUsrError, "Missing variable!");
                cleanup(CMfailed);
            }
            if (NCstringMatch(argv[argPos], 0, "VAR")) {
                CMmsgPrint(CMmsgUsrError, "Variable names cannot start with 'VAR'!");
                cleanup(CMfailed);
            }
            ...
            if (NCmathIsNumber(tmp)) NCmathSetVarVal(k, atof(tmp));
                ...
                CMmsgPrint(CMmsgUsrError, "%s is not a <double>!", tmp);
                cleanup(CMfailed);
            }
            free(tmp);
            free(name);
        ...
        if (CMargTest(argv[argPos], "-e", "--expression")) {
            ...
            if (input != (char *) NULL) {
                CMmsgPrint(CMmsgUsrError, "Expression defined twice!");
                cleanup(CMfailed);
            }
        ...
        if (CMargTest(argv[argPos], "-r", "--rename")) {
            if ((argNum = CMargShiftLeft(argPos, argv, argc)) <= argPos) {
                CMmsgPrint(CMmsgUsrError, "Missing name!");
                cleanup(CMfailed);
            }
            if (fieldname == (char *) NULL) {
                ...
                CMmsgPrint(CMmsgUsrError, "Output field name defined twice!");
                cleanup(CMfailed);
            }
            if ((argNum = CMargShiftLeft(argPos, argv, argc)) <= argPos) break;
            continue;
...
/*		CMmsgPrint (CMmsgUsrError, "<expression> =? ");
            ...
            if (inFile.ncTYPE) {
                ...
                for (k = 0; k < (inFile.tbl)->NFields; k++)
                    if (strcmp(((inFile.tbl)->Fields)[k].Name, tmp) == 0) {
                        if (colnum != -1) {
                            CMmsgPrint(CMmsgUsrError, "Multiple matches for column '%s'!", tmp);
                            free(tmp);
                            free(input);
                            cleanup(CMfailed);
                        }
                ...
                if (colnum == -1) {
                    ...
                    for (k = 0; k < (inFile.tbl)->NFields; k++)
                        if (strcmp(((inFile.tbl)->Fields)[k].Name, tmp) == 0) {
                            if (colnum != -1) {
                                printf("Multiple matches for column '%s'!", tmp);
                                free(tmp);
                                free(input);
                                cleanup(CMfailed);
                            }
                    ...
                    if (colnum == -1) {
                        CMmsgPrint(CMmsgUsrError, "Missing column '%s'!", tmp);
                        free(tmp);
                        free(input);
                        cleanup(CMfailed);
                    }
                }
            } else if (inFile.file == (FILE *) NULL) {
                CMmsgPrint(CMmsgUsrError, "No input file given!");
                cleanup(CMfailed);
            } else {
                colnum = -1;
    ...
    if (GetDebug()) {
        ...
        for (i = 0; i < NCmathGetVarNum(); i++) {
            if (NCmathGetVarVary(i))
                CMmsgPrint(CMmsgUsrError, "%s = colnum(%d)", NCmathGetVarName(i), NCmathGetVarColNum(i));
    ...
    if (isIneq(input, &i)) {
        ...
        if (GetDebug()) {
            CMmsgPrint(CMmsgUsrError, "Using equation: (");
            printInorderI(iHead, stderr);
            CMmsgPrint(CMmsgUsrError, ")");
        }
    } else {
        tHead = mkTree(input);
...
//			if (inFile.curRow == (char *) NULL) { CMmsgPrint (CMmsgUsrError, "Unexpected end of file!"); cleanup(CMfailed); }
            ...
            if (iHead) {
                ...
                output = Calculate(tHead);
                if (NCmathEqualValues(output, (double) ((int) output)))
