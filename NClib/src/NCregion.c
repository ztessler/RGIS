...
void NCregionInitialize(NCregion_t *region) {
    if (region == (NCregion_t *) NULL) {
        CMmsgPrint(CMmsgAppError, "Null Region Pointer in: %s %d", __FILE__, __LINE__);
        return;
    }
...
void NCregionExpand(NCregion_t *region0, NCregion_t *region1, NCregion_t *result) {
    if ((region0 == (NCregion_t *) NULL) || (region1 == (NCregion_t *) NULL) || (result == (NCregion_t *) NULL)) {
        CMmsgPrint(CMmsgAppError, "Null Region Pointer in: %s %s", __FILE__, __LINE__);
        return;
    }
    ...
    result->UpperRight.X =
            region0->UpperRight.X > region0->UpperRight.X ? region0->UpperRight.X : region0->UpperRight.X;
    result->UpperRight.Y =
            region0->UpperRight.Y > region0->UpperRight.Y ? region0->UpperRight.Y : region0->UpperRight.Y;
}
