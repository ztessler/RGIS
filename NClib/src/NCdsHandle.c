...
NCdsHandle_t *NCdsHandleOpenByIds(int *ncids, size_t n) {
    ...
    switch (dataType = NCdataGetType(ncids[0])) {
        ...
        case NCtypeGCont:
            dsHandle = (NCdsHandle_t *) malloc(sizeof(NCdsHandleGCont_t));
            break;
//		case NCtypeGDisc:    dsHandle = (NCdsHandle_t *) malloc (sizeof (NCdsHandleGDisc_t));    break;
        case NCtypeNetwork:
            dsHandle = (NCdsHandle_t *) malloc(sizeof(NCdsHandleNetwork_t));
            break;
        case NCtypePoint:
            dsHandle = (NCdsHandle_t *) malloc(sizeof(NCdsHandleVPoint_t));
            break;
...
//		case NCtypePolygon:  dsHandle = (NCdsHandle_t *) malloc (sizeof (NCdsHandleVPolygon_t)); break;
    ...
    switch (dataType) {
        default:
            ...
            return ((NCdsHandle_t *) NULL);
        case NCtypeGCont:
            ret = NCdsHandleGContDefine((NCdsHandleGCont_t *) dsHandle, ncids, n);
            break;
//		case NCtypeGDisc:   ret = NCdsHandleGDiscDefine    ((NCdsHandleGDisc_t *)    dsHandle, ncids, n); break;
        case NCtypeNetwork:
            ret = NCdsHandleNetworkDefine((NCdsHandleNetwork_t *) dsHandle, ncids[0]);
            break;
        case NCtypePoint:
            ret = NCdsHandleVPointDefine((NCdsHandleVPoint_t *) dsHandle, ncids[0]);
            break;
...
NCdsHandle_t *NCdsHandleOpen(const char *pattern) {
    ...
    if ((ncids = (int *) calloc(ncNum, sizeof(int))) == (int *) NULL) {
        CMmsgPrint(CMmsgSysError, "Memory allocation error in: NCdsHandleOpenByPattern ()");
        goto ABORT;
    }

    for (i = 0; i < ncNum; ++i)
        if ((status = nc_open(fileList[i], NC_NOWRITE, ncids + i)) != NC_NOERR) {
            CMmsgPrint(CMmsgUsrError, "Filename: %s", fileList[0]);
            NCprintNCError (status, "NCdsHandleOpenByPattern");
            ncids[i] = NCundefined;
            goto ABORT;
        }
...
NCdsHandle_t *NCdsHandleCreate(const char *pattern, const char *name, int dncid, NCtimeStep tsMode, ut_unit *tUnitIn,
    ...
    if ((searchStr = malloc(strlen(pattern) + 1)) == (char *) NULL) {
        CMmsgPrint(CMmsgSysError, "Memory allocation error in: NCdsHandleCreate ()!");
        return ((NCdsHandle_t *) NULL);
    }
    ...
    if ((utCalendar(eTime, tUnitIn, &endYear, &endMonth, &day, &hour, &minute, &second) != 0) ||
        ...
        CMmsgPrint(CMmsgAppError, "Calender scanning error in:%s %d", __FILE__, __LINE__);
        goto ABORT;
    }

    switch (tsMode) {
        case NCtimeYear:
            sprintf (tsUnitStr, "years since %04d-01-01 00:00 0.0", year);
            break;
        case NCtimeMonth:
            sprintf (tsUnitStr, "months since %04d-%02d-01 00:00 0.0", year, month);
            break;
        case NCtimeDay:
            sprintf (tsUnitStr, "days since %04d-%02d-%02d 00:00 0.0", year, month, day);
            break;
        case NCtimeHour:
            sprintf (tsUnitStr, "hours since %04d-%02d-%02d %02d:00 0.0", year, month, day, hour);
            break;
        case NCtimeMinute:
            sprintf (tsUnitStr, "minutes since %04d-%02d-%02d %02d:%02d 0.0", year, month, day, hour, minute);
            break;
        case NCtimeSecond:
            sprintf (tsUnitStr, "seconds since %04d-%02d-%02d %02d:%02d %.1f", year, month, day, hour, minute, second);
            break;
    }
    if (tsMode > NCtimeMonth) {
        if ((utScan(tsUnitStr, &tUnitOut) != 0) || (utConvert(tUnitIn, &tUnitOut, &scale, &offset) != 0)) {
            CMmsgPrint(CMmsgAppError, "Time unit scanning error in: %s %d", __FILE__, __LINE__);
            goto ABORT;
        }
    ...
    do {
        if (tsMode > NCtimeMonth) {
            if (utCalendar(sTime + time, &tUnitOut, &year, &month, &day, &hour, &minute, &second) != 0) {
                CMmsgPrint(CMmsgAppError, "Time unit scaning error in: %s %d", __FILE__, __LINE__);
                goto ABORT;
            }
        }
        ...
        for (i = 0; i < tsMode; ++i)
            if ((subStr = strstr(searchStr, str[i])) == (char *) NULL) break;
                ...
                switch (i) {
                    case NCtimeYear:
                        sprintf (subStr, "%04d", year);
                        subStr += 4;
                        strOffset -= 4;
                        break;
                    case NCtimeMonth:
                        sprintf (subStr, "%02d", month);
                        subStr += 2;
                        strOffset -= 2;
                        break;
                    case NCtimeDay:
                        sprintf (subStr, "%02d", day);
                        subStr += 2;
                        strOffset -= 2;
                        break;
                    case NCtimeHour:
                        sprintf (subStr, "%02d", hour);
                        subStr += 2;
                        strOffset -= 2;
                        break;
                    case NCtimeMinute:
                        sprintf (subStr, "%02d", minute);
                        subStr += 2;
                        strOffset -= 2;
                        break;
                    case NCtimeSecond:
                        sprintf (subStr, "%04.1f", second);
                        subStr += 4;
                        strOffset -= 4;
                        break;
                }
        ...
        if ((ncNum == 0) || (strcmp(fileNames[ncNum - 1], searchStr) != 0)) {
            ...
            if (((ncids[ncNum - 1] = NCfileCreate(fileNames[ncNum - 1], dncid)) == NCfailed) ||
                ...
                ((tvarid = NCdataGetTVarId(ncids[ncNum - 1])) == NCfailed))
                goto ABORT;
            index = 0;
        }
        if ((status = nc_put_var1_double(ncids[ncNum - 1], tvarid, &index, &time)) != NC_NOERR) {
            NCprintNCError (status, "NCdsHandleCreate");
            goto ABORT;
        }
        index++;
    ...
    for (i = 0; i < ncNum; i++) {
        nc_abort(ncids[i]);
        unlink(fileNames[i]);
        if (fileNames[i] != (char *) NULL) free(fileNames[i]);
    }
    utClear(&tUnitOut);
...
NCstate NCdsHandleClose(NCdsHandle_t *dsh) {
    switch (dsh->DataType) {
        default:
            break;
        case NCtypeGCont:
            NCdsHandleGContClear((NCdsHandleGCont_t *) dsh);
            break;
//		case NCtypeGDisc:    NCdsHandleGDiscClear    ((NCdsHandleGDisc_t *)    dsh); break; 
        case NCtypeNetwork:
            NCdsHandleNetworkClear((NCdsHandleNetwork_t *) dsh);
            break;
        case NCtypePoint:
            NCdsHandleVPointClear((NCdsHandleVPoint_t *) dsh);
            break;
...
NCstate NCdsHandleReference(const NCdsHandle_t *dsh, const NCcoordinate_t *coord, NCreference_t *ref) {
    switch (dsh->DataType) {
        default:
            break;
        case NCtypeGCont:
            return (NCdsHandleGContReference((const NCdsHandleGCont_t *) dsh, coord, ref));
//		case NCtypeGDisc:    return (NCdsHandleGDiscReference  ((const NCdsHandleGDisc_t *)  dsh, coord, ref));
        case NCtypePoint:
            return (NCdsHandleVPointReference((const NCdsHandleVPoint_t *) dsh, coord, ref));
    }
    return (NCfailed);
}
...
int NCdsHandleGetLNum(const NCdsHandle_t *dsh) {
    switch (dsh->DataType) {
        default:
            break;
        case NCtypeGCont:
            return (((const NCdsHandleGCont_t *) dsh)->LNum);
    }
    return (NCfailed);
}
...
int NCdsHandleGetTNum(const NCdsHandle_t *dsh) {
    switch (dsh->DataType) {
        default:
            break;
        case NCtypeGCont:
            return (((const NCdsHandleGCont_t *) dsh)->TNum);
    }
    return (NCfailed);
}
...
NCstate NCdsHandleUpdateRanges(const NCdsHandle_t *dsh) {
    switch (dsh->DataType) {
        default:
            break;
        case NCtypeGCont:
            return (NCdsHandleGContUpdateRanges((NCdsHandleGCont_t *) dsh));
    }
    return (NCfailed);
}
...
NCstate NCdsHandleLoadCache(NCdsHandle_t *dsh, size_t sTStep, size_t nTSteps, size_t sLevel, size_t nLevels) {
    switch (dsh->DataType) {
        default:
            break;
        case NCtypeGCont:
            return (NCdsHandleGContLoadCache((NCdsHandleGCont_t *) dsh, sTStep, nTSteps, sLevel, nLevels));
...
NCstate NCdsHandleSaveCache(NCdsHandle_t *dsh, size_t time, size_t level) {
    switch (dsh->DataType) {
        default:
            break;
        case NCtypeGCont:
            return (NCdsHandleGContSaveCache((NCdsHandleGCont_t *) dsh, time, level));
...
bool NCdsHandleGetFloat(const NCdsHandle_t *dsh, NCreference_t *ref, double *val) {
    switch (dsh->DataType) {
        default:
            break;
        case NCtypeGCont:
            return (NCdsHandleGContGetFloat((const NCdsHandleGCont_t *) dsh, ref, val));
...
NCstate NCdsHandleSetFloat(NCdsHandle_t *dsh, size_t *index, double val) {
    switch (dsh->DataType) {
        default:
            break;
        case NCtypeGCont:
            return (NCdsHandleGContSetFloat((NCdsHandleGCont_t *) dsh, index[0], index[1], val));
...
NCstate NCdsHandleSetFill(NCdsHandle_t *dsh, size_t *index) {
    switch (dsh->DataType) {
        default:
            break;
        case NCtypeGCont:
            return (NCdsHandleGContSetFill((NCdsHandleGCont_t *) dsh, index[0], index[1]));
//		case NCtypeGDisc:    return (NCdsHandleGDiscSetFloat ((NCdsHandleGDisc_t *)  dsh, index [0], index [1], val));
//		case NCtypePoint:    return (NCdsHandleVPoinSetFloat ((NCdsHandleVPoint_t *) dsh, index [0], val));
    }
    return (NCfailed);
...
NCstate NCdsHandleGetTime(const NCdsHandle_t *dsh, size_t layerID, ut_unit *tUnit, double *time) {
    switch (dsh->DataType) {
        ...
        case NCtypeGCont:
        case NCtypeGDisc:
            return (NCdsHandleGridGetTime((const NCdsHandleGrid_t *) dsh, layerID, tUnit, time));
    }
...
int NCdsHandleGetTimeStep(const NCdsHandle_t *dsh, size_t layerID, ut_unit *tUnit, double *tStep) {
    switch (dsh->DataType) {
        ...
        case NCtypeGCont:
        case NCtypeGDisc:
            return (NCdsHandleGridGetTimeStep((const NCdsHandleGrid_t *) dsh, layerID, tUnit, tStep));
    }
    CMmsgPrint(CMmsgAppError, "Data set does not have time series in: %s %d", __FILE__, __LINE__);
    return (NCfailed);
}
...
NCstate NCdsHandleGetTLayerID(const NCdsHandle_t *dsh, ut_unit *tUnit, double time) {
    switch (dsh->DataType) {
        ...
        case NCtypeGCont:
        case NCtypeGDisc:
            return (NCdsHandleGridGetTLayerID((const NCdsHandleGrid_t *) dsh, tUnit, time));
    }
    CMmsgPrint(CMmsgAppError, "Data set does not have time series in: %s %d", __FILE__, __LINE__);
    return (NCfailed);
}
