...
enum {
    NONE = 0, HOUR = -1, DAY = -2, MONTH = -3, YEAR = -4
};
    ...
    switch (i) {
        ...
        default:
            if ((t1 = (strlen(date1) > 3) && (sscanf(date1, "%2d", &t1) == 1) ? t1 * 365 * 24 : -1) == -1) {
                CMmsgPrint(CMmsgUsrError, "Problem with dates: %s and %s!", date1, date2);
                exit(-1);
            }
...
//	NCdsHandle_t *dsHandle;
    ...
    for (argPos = 1; argPos < argNum;) {
        ...
        if (CMargTest(argv[argPos], "-s", "--set")) {
            if ((argNum = CMargShiftLeft(argPos, argv, argc)) <= argPos) {
                CMmsgPrint(CMmsgUsrError, "Time step!");
                return (CMfailed);
            }

            if (interval != NONE) {
                CMmsgPrint(CMmsgUsrError, "Interval already set!");
                cleanup(NCfailed);
            }
            if (NCmathIsNumber(argv[argPos])) interval = atoi(argv[argPos]);
                ...
                CMmsgPrint(CMmsgUsrError, "Undefined time interval '%s'", argv[argPos]);
                cleanup(NCfailed);
            }
            if ((argNum = CMargShiftLeft(argPos, argv, argc)) <= argPos) {
            ...
            if (varname == (char *) NULL) varname = argv[argPos];
                ...
                CMmsgPrint(CMmsgUsrError, "Input field name defined twice!");
                cleanup(NCfailed);
            }
            if ((argNum = CMargShiftLeft(argPos, argv, argc)) <= argPos) break;
            continue;
    ...
    if (filename != (char *) NULL) {
        if (nc_open(filename, NC_WRITE, &ncid) != NC_NOERR) {
            CMmsgPrint(CMmsgAppError, "Error opening file: %s!", filename);
            return (NCfailed);
        }
