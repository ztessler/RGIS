...
DBInt RGPPrintError(DBInt mode, DBInt entryNum, const char *message) {
    if (mode == 0) {
        ...
        CMmsgPrint(CMmsgUsrError, "%s in line: %d\n", message, entryNum);
        return (true);
    }
}
...
int main(int argc, char **argv) {
    ...
    for (argPos = 1; argPos < argNum;) {
        if (CMargTest (argv[argPos], "-m", "--mode")) {
            ...
            if ((mode = CMoptLookup(modes, argv[argPos], true)) == DBFault) {
                CMmsgPrint(CMmsgUsrError, "Invalid mode %s", argv[argPos]);
                goto Usage;
            }
        ...
        if (CMargTest (argv[argPos], "-d", "--device")) {
            ...
            if ((device = CMoptLookup(devices, argv[argPos], true)) == DBFault) {
                CMmsgPrint(CMmsgUsrError, "Invalid device %s", argv[argPos]);
                goto Usage;
            }
            if ((argNum = CMargShiftLeft(argPos, argv, argNum)) <= argPos) break;
            continue;
        ...
        if (CMargTest (argv[argPos], "-p", "--psize")) {
            ...
            if ((argv[argPos] == (char *) NULL) || (sscanf(argv[argPos], "%f,%f", &pWidth, &pHeight) != 2)) {
                CMmsgPrint(CMmsgUsrError, "Invalid page size %s", argv[argPos]);
                goto Usage;
            }
            if ((argNum = CMargShiftLeft(argPos, argv, argNum)) <= argPos) break;
            continue;
        ...
        if (CMargTest (argv[argPos], "-f", "--format")) {
            ...
            if ((format = CMoptLookup(formats, argv[argPos], true)) == DBFault) {
                CMmsgPrint(CMmsgUsrError, "Invalid format %s", argv[argPos]);
                goto Usage;
            }
            if ((argNum = CMargShiftLeft(argPos, argv, argNum)) <= argPos) break;
            continue;
        ...
        if (CMargTest (argv[argPos], "-l", "--layout")) {
            ...
            if ((layout = CMoptLookup(layouts, argv[argPos], true)) == DBFault) {
                CMmsgPrint(CMmsgUsrError, "Invalid layout %s", argv[argPos]);
                goto Usage;
            }
            if ((argNum = CMargShiftLeft(argPos, argv, argNum)) <= argPos) break;
            continue;
    ...
    do {
        ...
        else if (RGPPrintError(mode, entryNum, "Panel layout input error")) goto Stop;
    } while (true);
    ...
    for (panelRow = 0; panelRow < panelRowNum; ++panelRow)
        for (panelCol = 0; panelCol < panelColNum; ++panelCol) {
            ...
            do {
                ...
                RGPPrintError(mode, entryNum, "Panel Title input error");
                goto Stop;
            } while (true);
            ...
            do {
                ...
                if ((fgets(charBuffer, sizeof(charBuffer) - 1, stdin) != (char *) NULL) &&
                    ...
                    if (dbData->Read(charBuffer) != DBSuccess) {
                        dataNum--;
                        continue;
                    }
                    switch (dbData->Type()) {
                        ...
                        case DBTypeNetwork:
                            ...
                            break;
                        default:
                            CMmsgPrint(CMmsgUsrError, "Invalid data type");
                            dataNum--;
                            break;
                    }
                    delete dbData;
