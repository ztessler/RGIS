...
int NCstringEndPar(char *expr, int i) { // returns the number of the char in array of the other side of the parentheses
...
NCstate NCstringReplace(char **input, int s, int e, char *newstr) {
    ...
    char *ret;
    if (e < s) {
        CMmsgPrint(CMmsgUsrError, "NCstringReplace(): end is less than start!\n");
        abort();
    }
